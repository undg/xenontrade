...
class Parser {
    ...
    fixCannotUseText() {
        ...
        for (var i in lines) {
            if (
                lines[i].includes("You cannot use this item. Its stats will be ignored")
            ) {
                index = i;
                break;
            }
    ...
    _getUniqueVariant() {
        ...
        if (itemVariants.hasOwnProperty(name)) {
            if (itemVariants[name].hasOwnProperty("regex")) {
                var match = this.clipboard.match(
                    new RegExp(itemVariants[name].regex.pattern)
                );
                ...
                for (var index in itemVariants[name].mods) {
                    ...
                    for (var regexIndex in mods.regex) {
                        var match = this.clipboard.match(
                            new RegExp(mods.regex[regexIndex])
                        );
    ...
    _getGemVariant() {
        ...
        for (var i = 0; i < gemVariants.length; i++) {
            if (
                ...
                (corrupted === gemVariants[i].corrupted ||
                    gemVariants[i].corrupted === null) &&
    ...
    getLinks() {
        ...
        if (socketString !== "") {
            ...
            if (linkCount === 5) {
                ...
                if (
                    sockets.length === 3 &&
                    (sockets[0].length === 1 || sockets[1].length === 1)
                ) {
                    return 5;
                }
            }
