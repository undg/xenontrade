...
class GUI {
    ...
    static _initializeButtons() {
        $(".menu")
            .find("[data-button='minimize']")
            .click(() => GUI.hide());
        $(".menu")
            .find("[data-button='close']")
            .click(() => GUI.close());
        $(".menu")
            .find("[data-button='update']")
            .click(() => Pricecheck.updateNinja());

        $(".menu")
            .find("[data-button='buy']")
            .click(() => PathOfExile.chat(electron.clipboard.readText()));
        $(".menu")
            .find("[data-chat]")
            .click((e) => PathOfExile.chat(e.currentTarget.dataset.chat));

        $(".menu")
            .find("[data-button='settings']")
            .click(() => GUI.toggleSettingsWindow());
        $(".menu")
            .find("[data-button='lock']")
            .click(() => GUI.toggleLock());
        $(".menu")
            .find("[data-button='close-all']")
            .click(() => GUI.closeAllEntries());
        $(".menu")
            .find("[data-button='submenu']")
            .click((e) => GUI.toggleSubmenu(e));
    ...
    static onFocus() {
        var timeout = setTimeout(function () {
            ...
            // If...
            if (
                config.get("focusPathOfExile") && // ... Always focus Path of Exile is enabled
                ...
                // ... Focused element is not a textarea
                PathOfExile.focus();
                Helpers.setAlwaysOnTop();
            }
        }, 20);
    ...
    static onEntriesChange() {
        ...
        if (config.get("hideMenu")) {
            ...
            if (!hasEntries && win.isVisible()) {
                GUI.hide(true);
            } else if (hasEntries && !win.isVisible()) {
                GUI.show(true);
            }
        }
    }
    ...
    static showUpdateAvailableEntry(info) {
        var updateEntry = new TextEntry(
            "Update available",
            "A new version of XenonTrade (<span>v" +
            info.version +
            "</span>) is available.<br /><i class='fas fa-arrow-right'></i> <span data-entry-link='download'>Update now</span>",
            { icon: "fa-box blue" }
        );
        ...
        $(".entry[data-id='" + updateEntry.getId() + "']")
            .find("[data-entry-link='download']")
            .click(function () {
                ipcRenderer.send("download-update");

                updateEntry.setTitle(
                    "Downloading <span>v" + info.version + "</span>..."
                );
                updateEntry.setText("");
    ...
    static showUpdateDownloadedEntry(info) {
        if (entries.hasOwnProperty("update-entry")) {
            ...
            updateEntry.setText(
                "XenonTrade <span>v" +
                info.version +
                "</span> has been downloaded. It will be automatically installed after closing XenonTrade.<br /><i class='fas fa-arrow-right'></i> <span data-entry-link='install'>Install now</span>"
            );
