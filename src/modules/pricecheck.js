...
class Pricecheck {
  ...
  static _getNinjaPrice(parser) {
    if (ninjaAPI.hasData(config.get("league"))) {
      ninjaAPI
        .getItem(parser.getName(), {
          league: config.get("league"),
          links: parser.getLinks(),
          variant: parser.getVariant(),
          fallbackVariant: parser.getDefaultVariant(),
          relic: parser.isRelic(),
          baseType: parser.getBaseType(),
        })
        .then((itemArray) => {
      ...
      new TextEntry(
        ...
        "There's no data for " +
          config.get("league") +
          ". You should update before attempting to price check another item.",
        { icon: "fa-exclamation-triangle yellow", timeout: 10 }
      ).add();
    }
  }
  ...
  static _getRarePrice(parser) {
    var entry = new TextEntry("Getting price prediction...", {
      closeable: false,
    });
    entry.add();
  ...
  static updateNinja() {
    if (!ninjaAPI.isUpdating()) {
      ...
      ninjaAPI
        .update({ league: config.get("league") })
        .then((result) => {
