...
class NinjaAPI {
    ...
    update(options) {
        ...
        return new Promise(function (resolve, reject) {
            if (!self.updating) {
                self.updating = true;
                var promises = self._getRequestCallsArray(
                    options.league,
                    options.delay
                );
    ...
    _processRequest(contents, api, league, resolve, reject) {
        if (isValidNinjaApi(contents)) {
            ...
            reject(
                new Error(
                    "The data from the requested " +
                    api.type +
                    " API (League: " +
                    league +
                    ") could not be processed because the format is invalid or the response is empty. Possible reasons: 1) Invalid league name, 2) poe.ninja is down, 3) poe.ninja changed their API structure"
                )
            );
        }
    }
    ...
    _getMatchesInType(type, name, options) {
        var overview = getOverviewByType(type, this.apis);
        var matches = [];
        ...
        if (this._hasDataForTypeInLeague(type, options.league)) {
            ...
            matches = addApiTypeToMatches(type, matches);
        }
    ...
    getCurrencyDetails(name) {
        ...
        if (this.data.hasOwnProperty("CurrencyDetails")) {
            var matches = this.data.CurrencyDetails.filter(function (item) {
...
function hasPropertyWithData(obj, prop) {
    return !(
...
function getOverviewByType(type, apis) {
...
function isValidNinjaApi(obj) {
    return hasProperty(obj, "lines");
}
function hasCurrencyDetailsData(obj) {
    return hasPropertyWithData(obj, "currencyDetails");
}
function addApiTypeToMatches(type, matches) {
    matches = matches || [];
