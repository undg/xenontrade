      ...
      request(url + parameters, { json: true })
        .then((response) => {
          if (!PoePrices.hasAllKeys(response) || response.error !== 0) {
            var requestObject = {
              request: { parameters: parsedParams, itemText },
              response,
            };

            log.warn(
              "Request to poeprices.info failed. Received an empty response.\n" +
        ...
        .catch((error) => {
          log.warn(
            "Request to poeprices.info failed.\n" +
              JSON.stringify(error, null, 4)
          );
          reject(
            new Error("Request to <b>poeprices.info</b> failed. " + error.error)
          );
        });
    });
  }
